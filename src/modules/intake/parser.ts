...
export async function runIntake(
  input: IntakeInput,
  intakeDir: string = CONFIG.paths.intake,
): Promise<{ outputPath: string; output: IntakeOutput }> {
...
async function analyseRequirements(input: IntakeInput): Promise<IntakeOutput> {
    ...
    `\n\n以下為 IntakeOutput 範例，請對照填寫：
{
  ...
  for (let attempt = 0; attempt < CONFIG.ai.totalAttempts; attempt++) {
    try {
      ...
      const raw = JSON.parse(text) as IntakeOutput;

      // Normalize and validate sub-structures
      const { mvp, niceToHave } = normalizeFeatures(
        raw.mvp_features,
  ...
  throw new AIParseError(
    `AI analysis failed after ${CONFIG.ai.totalAttempts} attempts: ${lastError?.message}`,
    CONFIG.ai.totalAttempts,
  );
}
