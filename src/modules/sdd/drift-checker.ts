...
const COVERAGE_NOTE =
  "AI 分析不保證 100% 覆蓋，對複雜邏輯可能誤判。建議搭配人工審查，不作為唯一驗收依據。";
...
async function collectImplementation(implPath: string): Promise<string> {
  const files = new Map<string, string>();
  try {
    const entries = await fs.readdir(implPath, { recursive: true });
    for (const entry of entries) {
      const entryStr = String(entry);
      if (!entryStr.endsWith(".ts")) continue;
      const fullPath = path.join(implPath, entryStr);
      const content = await readFile(fullPath);
      if (content) files.set(entryStr, content);
    }
  } catch {
    // implPath may not exist or may be inaccessible
  }

  if (files.size === 0) return "(no implementation files found)";
...
async function analyseWithAI(
  acList: { id: string; description: string; test_method: string }[],
  implSummary: string,
): Promise<DriftItem[]> {
  const client = new Anthropic();

  const acJson = JSON.stringify(
    acList.map((a) => ({ id: a.id, condition: a.description })),
    null,
    2,
  );

  const systemPrompt =
    "你是一位程式碼審查專家，負責驗證實作是否符合規格文件的驗收標準（AC）。\n" +
  ...
  let results: AIACResult[] = [];
  try {
    // Strip potential markdown fences
    const cleaned = text.replace(/^```[a-z]*\n?/m, "").replace(/\n?```$/m, "");
    results = JSON.parse(cleaned) as AIACResult[];
  } catch {
