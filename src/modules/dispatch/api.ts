...
import type { TaskRecord } from "../../types/dispatch.js";
...
export async function handleDispatchRequest(
  ...
  if (method === "POST" && pathname === "/api/dispatch") {
    try {
      ...
      if (rawModel && (["opus", "sonnet", "haiku"] as const).includes(rawModel as "opus")) {
        command = { ...command, model: rawModel as "opus" | "sonnet" | "haiku" };
      }
