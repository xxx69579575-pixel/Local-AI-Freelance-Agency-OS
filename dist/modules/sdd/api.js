...
function deriveCurrentPhase(phases) {
    ...
    if (inProgress.length > 0) {
        return inProgress.sort((a, b) => PHASE_ORDER.indexOf(a.phase) - PHASE_ORDER.indexOf(b.phase))[0].phase;
    }
    const done = phases.filter((p) => p.status === "done");
    if (done.length === 0)
        return "1.1";
    const lastDone = done.sort((a, b) => PHASE_ORDER.indexOf(b.phase) - PHASE_ORDER.indexOf(a.phase))[0];
...
async function computePhaseStatuses() {
...
async function anyFileExists(dir, ext) {
    try {
...
async function buildSpecSummaries(statusFilter) {
    const summaries = [];
    let entries = [];
    try {
    ...
    for (const entry of entries) {
        if (!entry.endsWith(".md") || entry === "review")
            continue;
        ...
        if (statusFilter && statusFilter !== reviewStatus)
            continue;
 ...
 */
export async function handleSddRequest(req, res) {
    const url = req.url ?? "";
    ...
    if (method === "POST" && url === "/api/sdd/spec-gate") {
        try {
            ...
            logger.error("spec-gate error", { error: err.message });
    ...
    if (method === "GET" && url === "/api/sdd/status") {
        try {
            ...
            logger.error("sdd status error", { error: err.message });
            send(res, 500, { error: "Internal server error" });
        }
        return true;
    }
    ...
    if (method === "GET" && (url === "/api/sdd/specs" || url.startsWith("/api/sdd/specs?"))) {
        try {
            ...
            logger.error("sdd specs error", { error: err.message });
            send(res, 500, { error: "Internal server error" });
        }
        return true;
    }
    ...
    if (method === "POST" && url === "/api/sdd/drift-check") {
        try {
            ...
            logger.error("drift-check error", { error: err.message });
            send(res, 500, { error: "Internal server error" });
        }
        return true;
    }
