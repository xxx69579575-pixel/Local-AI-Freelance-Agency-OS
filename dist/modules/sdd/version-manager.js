 ...
 * Targets: > **文件版本**：v1.0
 */
export function replaceVersion(content, newVersion) {
    return content.replace(/(\*\*文件版本\*\*[：:])\s*v\d+\.\d+/, `$1 ${newVersion}`);
}
/**
 ...
 */
export function appendChangelog(content, version, date, entries) {
    const entryText = `### ${version} — ${date}\n` +
