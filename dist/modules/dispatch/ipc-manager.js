 ...
 */
export async function askAndPause(taskSlug, seq, question, background, options) {
 ...
 */
export async function checkPendingAnswer(taskSlug, seq) {
    const answerPath = path.join(ipcDir(taskSlug), `${seqStr(seq)}.answer`);
    if (!(await fileExists(answerPath)))
        return null;
 ...
 */
export async function writeAnswer(taskSlug, seq, answer) {
 ...
 */
export async function isQuestionTimedOut(taskSlug, seq) {
 ...
 */
export async function readQuestion(taskSlug, seq) {
    const questionPath = path.join(ipcDir(taskSlug), `${seqStr(seq)}.question`);
    return readFile(questionPath);
}
function buildQuestionContent(msg) {
    const lines = [
        `**問題**：${msg.question}`,
        `**背景**：${msg.background}`,
    ];
